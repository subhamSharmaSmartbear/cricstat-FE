import React from 'react'
import rohit from "../../assets/rohitSharma.svg"
import dewald from "../../assets/dewald.svg"
import airplane from "../../assets/airplane.svg"
const Squad = () => {
  return (
    <div className='w-[100%]  h-[60%] flex flex-wrap -mx-2 gap-[1rem] justify-between overflow-y-scroll  scrollable-content custom-scrollbar  '>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={rohit} className='w-[100%] h-[100%] object-cover' alt='profile'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate'>Rohit Sharma </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={dewald} className='w-[100%] h-[100%] object-cover'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate flex w-[100%] items-center justify-center gap-[1rem]'> <img src={airplane} alt='airplane' /> Dewald Brewis </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={rohit} className='w-[100%] h-[100%] object-cover'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate'>Rohit Sharma </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={rohit} className='w-[100%] h-[100%] object-cover'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate'>Rohit Sharma </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={rohit} className='w-[100%] h-[100%] object-cover'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate'>Rohit Sharma </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={rohit} className='w-[100%] h-[100%] object-cover'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate'>Rohit Sharma </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={rohit} className='w-[100%] h-[100%] object-cover'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate'>Rohit Sharma </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={rohit} className='w-[100%] h-[100%] object-cover'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate'>Rohit Sharma </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      <div className='w-[280px] h-[350px] bg-[#434343] rounded-[10px] justify-self-start rounded-[10px]'>
        <div className='w-[100%] h-[80%] '>
          <img src={rohit} className='w-[100%] h-[100%] object-cover'/>
        </div>
        <div className='w-[100%] h-[20%] text-white text-center flex flex-col'>
          <span className='text-[22px] truncate'>Rohit Sharma </span>
          <span className='text-[#E0E0E0]'>Batter</span>
        </div>
      </div>
      
      
      
      
      
     
    </div>
  )
}

export default Squad
